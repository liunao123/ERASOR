...
vector<float> split_line(string input, char delimiter) {
    ...
    while (getline(ss, temp, delimiter)) {
        // cout<<"temp: "<< temp <<endl;
        answer.push_back(std::stof(temp));
    }
    return answer;
}
...
void load_all_poses(string txt, vector<Eigen::Matrix4f >& poses){
    ...
    while (std::getline(in, line)) {
        ...
        vector<float> pose = split_line(line.substr(16)  , ' ');

        Eigen::Translation3f ts(pose[1], pose[2], pose[3]);
        Eigen::Quaternionf q(pose[7], pose[4], pose[5], pose[6]);
...
int main(int argc, char **argv)
{
    ...
    string raw_map_path = DATA_DIR + "/saveMap_all.pcd";
    string pose_path = DATA_DIR + "/pose_graph/graph.g2o";
    string pcd_dir = DATA_DIR + "/pose_graph/";
    // Load raw pointcloud
    ...
    for (int i = INIT_IDX; i < N; ++i) {
        ...
        string pcd_name = pcd_dir + (boost::format("%06d")  % i ).str() + "/cloud.pcd";

        cout<<" pcd_name : "<<  pcd_name <<endl;
